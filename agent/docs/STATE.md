# ALR — Current State
Last updated: 2026-08-23 by p1-005-006-session
...
Tidak ada yang in-progress — P1-005+P1-006 baru saja selesai. Next up: P1-007+P1-008+P1-009 (Learning Event → Mastery → FRSS) — lihat "Next action". **Ini bagian paling kritis & paling perlu ketelitian di Phase 1.**
...
- **P1-005+P1-006: Assessment API + Attempt Submit/Scoring** — `GET /assessments/{id}`, `POST /assessments/{id}/attempts` (tidak pernah bocorkan `correct_answer`, tolak attempt in-progress duplikat), `POST /attempts/{id}/submit` (auto-grade mcq/fill_blank via `service/grading.rs`, tolak submit ganda/kurang jawaban) jalan penuh, 10 integration test + 4 unit test grading lulus (total 43 test) + smoke test manual end-to-end lewat curl (skor 100 buat semua benar, 422 `missing` yang isinya benar, 409 submit ganda — semua cocok kontrak). **`learning_events_created` di response submit sengaja masih hardcode `0`** — itu tugas P1-007 berikutnya, bukan bug. Detail lengkap di `docs/tickets/phase-1.md`.
...
2. **P1-007 (Learning Event Writer)** — sesi berikutnya, depends on P1-006 (done). Setiap submit attempt harus bikin 1 `learning_event` per soal (event_type=`question_answered`, payload minimal `{correct, difficulty, question_id, concept_ids}`) — bukan 1 event per attempt. Begitu ini jalan, **wajib** balikin ke `assessment_service::submit_attempt` dan isi `learning_events_created` dengan count asli (sekarang di-hardcode `0`, lihat catatan P1-006 di `docs/tickets/phase-1.md`). Pakai `question_concepts` (sudah ada dari P0-007) buat dapetin `concept_ids` per soal.
3. Lanjut P1-008 (Mastery Calculator v1) + P1-009 (FRSS Scheduler v1) — **paling perlu ketelitian** di seluruh Phase 1, karena formula ADR-0002/0003 harus presisi ke angka contoh manual di ADR-nya (DoD P1-008 eksplisit: unit test harus hasilkan `mastery_score = 72` persis, bukan kira-kira). Baca `docs/adr/0002-mastery-formula.md` dan `docs/adr/0003-frss-algorithm.md` pelan-pelan sebelum mulai coding, jangan cuma dari ingatan/asumsi.
...
7. P0-010 (CI) — sekarang berarti CI config per repo (`titian-web`, `titian-backend`, nanti `titian-mobile`), bukan satu CI monorepo. `titian-backend` sekarang punya 43 test (`cargo test`, butuh `DATABASE_URL` ke Postgres yang bisa CREATEDB) yang layak masuk CI. Pertimbangkan juga `docker-compose.yml` untuk `titian-backend` supaya onboarding tim tidak perlu `docker run` manual.
