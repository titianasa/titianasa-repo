# ALR — Current State
Last updated: 2026-08-23 by p1-003-004-session
...
Tidak ada yang in-progress — P1-003+P1-004 baru saja selesai. Next up: P1-005 + P1-006 (Assessment + Attempt submit/scoring) — lihat "Next action".
...
2. **P1-005 + P1-006 (Assessment API + Attempt Submission/Scoring)** — sesi berikutnya. Endpoint: `GET /assessments/{id}`, `POST /assessments/{id}/attempts`, `POST /attempts/{id}/submit`. Depends on P1-003+P1-004 (done). Perlu tambah seed `assessments`+`assessment_questions` ke `seed.sql`. P1-006 auto-grading cuma buat `mcq`/`fill_blank` (exact/normalized match) — tipe soal lain (writing/speaking) di-skip dari auto-score sesuai AC ticket, status attempt tetap `submitted` bukan `evaluated`. Attempt submit ganda harus 409, bukan 200/500.
3. Lanjut P1-007+P1-008+P1-009 (learning event → mastery → FRSS, **paling perlu ketelitian** karena formula ADR-0002/0003 harus presisi ke angka contoh manual di ADR-nya — DoD eksplisit minta unit test yang hasilnya cocok persis ke contoh, mis. `mastery_score = 72`).
...
7. P0-010 (CI) — sekarang berarti CI config per repo (`titian-web`, `titian-backend`, nanti `titian-mobile`), bukan satu CI monorepo. `titian-backend` sekarang punya 29 test (`cargo test`, butuh `DATABASE_URL` ke Postgres yang bisa CREATEDB) yang layak masuk CI. Pertimbangkan juga `docker-compose.yml` untuk `titian-backend` supaya onboarding tim tidak perlu `docker run` manual.
