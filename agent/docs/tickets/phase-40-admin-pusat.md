...
## Halaman Peserta (di luar penomoran P40, dikerjakan 2026-09-13 setelah user meminta "dashboard user monitoring" sebagai bagian checklist MVP launch)

✅ **SELESAI (2026-09-13, Sonnet).** Mengisi placeholder "Peserta" yang disiapkan P40-001 dan dijadwalkan ADR-0014 §6 langkah 5 ("Setelah Fase 39") — Fase 39 baru saja selesai total sesi ini, jadi placeholder ini sudah bisa diisi. Ini BUKAN tiket P40-00X baru (tidak melalui pipeline rencana Opus/tiket tertulis), melainkan permintaan langsung user di luar Phase 39/40 — didokumentasikan di sini karena secara teknis melengkapi sidebar Admin Pusat P40-001.

**Yang sudah diantisipasi ADR-0014 §2** (DAU/WAU/MAU, corong onboarding, retensi kohort, sebaran jenjang/organisasi) dibangun mengikuti pola P40-004/005 PERSIS: 2 tabel agregat baru (`metrics_daily_participants`, `metrics_weekly_retention`) dihitung ulang oleh job `metrics_rollup` yang SUDAH ADA (bukan job baru), dibackfill lewat binary `backfill_metrics` yang sudah ada juga.

**Yang TIDAK ada di ADR-0014 dan dibangun baru** — infrastruktur tracking, bukan sekadar halaman baca data: `users` sebelumnya tidak punya kolom aktivitas/login sama sekali.
- `user_login_events` (satu baris per login Google OAuth sukses: IP, User-Agent, device label) dan `user_presence` (satu baris per user, di-upsert) — migrasi `0053_user_presence_participants.sql`.
- **"Sedang online"**: `user_presence.last_seen_at` disentuh di `middleware/auth.rs::auth_middleware` PADA SETIAP request terautentikasi, tapi lewat *conditional upsert* (`on conflict do update ... where last_seen_at < now() - interval '2 menit'`) — Postgres sendiri yang melewati penulisan kalau baris masih segar, jadi murah dipanggil di jalur panas tanpa perlu Redis. "Online" = `last_seen_at` dalam 5 menit terakhir, dibaca LANGSUNG (bukan dari agregat) — presence berubah tiap menit, job per jam akan selalu bohong.
- **IP + device**: `ConnectInfo<SocketAddr>` (peer TCP asli, BUKAN header `X-Forwarded-For` — repo ini tidak punya reverse proxy dikonfigurasi di mana pun, jadi sebuah header cuma klien mengaku-aku) diwire di `main.rs` lewat `into_make_service_with_connect_info`. **Bug nyata ditemukan+diperbaiki**: `ConnectInfo<SocketAddr>` wajib (bukan `Option`) bikin SEMUA test integrasi yang lewat `Router::oneshot` gagal 500 — `oneshot` tidak pernah lewat listener sungguhan jadi extension `ConnectInfo` tidak pernah ada. Axum juga TIDAK mendukung `Option<ConnectInfo<T>>` sebagai extractor langsung (`ConnectInfo` tidak mengimplementasikan `OptionalFromRequestParts`-nya axum) — diperbaiki dengan extractor kustom `MaybeConnectInfo` yang baca extension manual & tidak pernah gagal, dipakai di `handlers/auth.rs::google_callback`.
- Device label dari User-Agent lewat crate `woothee` (pattern bawaan, tanpa file data eksternal) — diverifikasi hidup: sesi browser QA sungguhan tercatat `"Linux · Chrome"` dengan benar.
- **Region (daerah)**: **keputusan user eksplisit** — lookup LIVE ke API pihak ketiga gratis (`ip-api.com`, lewat `reqwest` yang sudah jadi dependency, tanpa crate baru), tapi HANYA saat admin membuka halaman detail satu pengguna (bukan di jalur login/request panas), hasilnya di-cache 24 jam di `user_presence.region`. IP privat/loopback (termasuk semua IP dev) dilewati sama sekali tanpa panggilan keluar (`is_private_or_loopback`) — diverifikasi test tidak butuh jaringan sama sekali untuk kasus ini.
- **Audit log per pembukaan data individu** (ADR-0014 §5 eksplisit mewajibkan ini): `GET /admin/participants/{user_id}` selalu memanggil `admin_audit::record` dengan action `"participant.viewed"`, tanpa syarat — diverifikasi lewat test DAN lewat curl langsung ke server dev sungguhan.
- **"Aktivitas"**: tidak ada infrastruktur baru — `learning_events` (Fase 39) sudah jadi log aktivitas lengkap per user, tinggal dibaca 50 terakhir per `user_id`.

**Dibuktikan lewat 5 test integrasi baru** (`admin_participants_test.rs`): gerbang izin 403 di ketiga endpoint baru; pencarian by nama ATAU email, case-insensitive; detail lengkap (presence+riwayat login+aktivitas) DAN baris audit log tertulis dalam satu request; "sedang online" cuma menghitung presence <5 menit (baris 10 menit basi tidak ikut terhitung); DAU/WAU/MAU baca dari tabel agregat (bukan scan langsung) dengan "belum cukup data" yang benar saat tidak ada baris periode sebelumnya.

**Dibuktikan lewat backfill + verifikasi manual di DB dev sungguhan**: `backfill_metrics` menghasilkan 14 hari data participants, DAU hari ini (0) cocok dengan `count(distinct user_id)` manual ke `learning_events`; kohort retensi minggu ini (23 pendaftar, 0 aktif) diverifikasi manual memang benar 0 (bukan bug query) — semua 23 user kohort itu genuinely tidak punya baris `learning_events` sama sekali.

**Dibuktikan lewat browser sungguhan** (`Bun.WebView`, akun `platform_admin` asli dengan token QA sementara, dihapus setelah selesai): halaman `/admin/peserta` menampilkan tile DAU/WAU/MAU/online, corong, tabel retensi berwarna, sebaran jenjang/organisasi — semua dengan data dev sungguhan; kotak pencarian menemukan pengguna & mengarah ke halaman detail; halaman detail menampilkan profil, status online, perangkat/IP/lokasi, riwayat login, aktivitas. **Sesi browser QA itu sendiri membuktikan pipeline presence end-to-end secara langsung**: baris `user_presence` akun admin muncul dengan `last_ip='127.0.0.1'` dan `last_device_label='Linux · Chrome'` — persis cocok dengan browser Chrome headless (backend `Bun.WebView`) yang dipakai, bukan data buatan.

`cargo test --lib` 229 lulus (tidak berubah). `cargo test --test integration` penuh: **154 lulus, 26 gagal — daftar sama persis**, tidak bertambah (154 = 149 sebelumnya + 5 baru; 3 test `auth_test` sempat gagal 500 gara-gara bug `ConnectInfo` di atas, diperbaiki dan dikonfirmasi lulus lagi sebelum dianggap selesai). `npx tsc --noEmit` dan `npx eslint` bersih untuk semua berkas frontend yang disentuh.

- Backend baru: `migrations/0053_user_presence_participants.sql`, `src/services/request_meta.rs`, `src/services/admin_participants.rs`. Diperluas: `main.rs`, `middleware/auth.rs`, `services/auth.rs`, `handlers/auth.rs`, `services/metrics_rollup.rs`, `src/bin/backfill_metrics.rs` (otomatis lewat `rollup_day`/`backfill` yang sudah ada), `routes/admin.rs`, `handlers/admin.rs`.
- Frontend baru: `app/admin/peserta/[userId]/page.tsx`. Diperluas: `app/admin/peserta/page.tsx` (placeholder diganti halaman nyata), `hooks/use-admin-metrics.ts`, `lib/api-client.ts`, `components/admin/admin-shell.tsx` (badge "Fase 39" dihapus dari entri Peserta).

DoD: dashboard menampilkan DAU/WAU/MAU/retensi/sebaran dengan data dev sungguhan; "sedang online" berubah nyata saat sesi baru login; detail pengguna menampilkan IP/device/region/riwayat login/aktivitas dan tercatat di log audit setiap dibuka.
