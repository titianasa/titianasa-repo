...
**Acceptance Criteria:**
- [x] `payment_provider.ts` baru: interface `PaymentProvider { createPayment(orderId, amountIdr): Promise<{paymentId, qrisPayload}>; }`. `StubQrisProvider` — balikin `payment_id` acak + `qris_payload` dummy, TIDAK pernah hit network asli. Provider asli (Midtrans/dst) TIDAK diimplementasi ticket ini — placeholder yang jelas ditandai di kode + `docs/STATE.md` sebagai keputusan eksplisit user, bukan lupa
- [x] Migrasi baru: `orders` (`id`, `enrollment_id` FK unique, `amount_idr` bigint, `status` CHECK IN (`pending`,`paid`,`failed`,`refunded`), `payment_id` text nullable, `created_at`)
- [x] `POST /enrollments/{id}/checkout` — buat `orders` row (`status=pending`) + panggil `PaymentProvider.createPayment`, balikin `qris_payload` ke client
- [x] `POST /payments/{id}/webhook` — simulasi callback sukses (mode stub: endpoint ini yang biasanya provider asli panggil, di sini dipanggil manual test/dev) — `orders.status → paid`, `enrollments.status → active`, DAN split 30/70: insert `transactions` (`type: payout_earned`, `user_id`: tutor, `amount`: 70% `amount_idr`, **satuan Rupiah mentah, bukan credit** — lihat "Keputusan scope" soal ambiguitas unit) + platform fee 30% dicatat sebagai `reference` di baris yang sama (bukan baris terpisah — platform tidak punya `users` row buat jadi `transactions.user_id`-nya sendiri, jadi fee dicatat sebagai metadata bukan baris ledger kedua)
- [x] `POST /enrollments/{id}/cancel` — kebijakan §8.14 (contoh dari sumber, didokumentasikan sebagai default tunable): student cancel >24 jam sebelum `cohorts.starts_at` → full refund; 6-24 jam → 50%; <6 jam → no refund. Refund → `orders.status = refunded` + `transactions` baru `type: refund` ke SISWA (bukan tutor) — TIDAK reverse `payout_earned` tutor kalau sudah dibayarkan (kompleksitas clawback di luar scope ticket ini, dicatat eksplisit sebagai keterbatasan, bukan silently ignored)
- [x] `GET /tutors/me/wallet` — SUM `transactions` (`payout_earned` - `payout_withdrawn`) buat tutor itu, pola sama `credits.balance`/`user_xp.total` (cached read, bukan scan tiap kali — TAPI karena ini tabel baru dipakai, agregasi on-read dulu, cache belakangan kalau perlu, sama pola P8-005's leaderboard)
...
**Catatan implementasi:**
1. **`AppState` dapat field baru `paymentProvider`** — sama pola `aiProvider`/`storage`, wajib (bukan optional), disuntik lewat `buildApp()`. Ini nyentuh **40 file test** yang semuanya manggil `buildApp({...})` — semua ditambah `paymentProvider: testPaymentProvider()` (helper baru `tests/helpers/payment-provider.ts`, wrapping `StubQrisProvider` — sama pola `testAiProvider()` wrapping `FakeAIProvider`). `src/index.ts` (entry point produksi) juga dapat `paymentProvider = new StubQrisProvider()` — stub-nya dipakai di produksi juga sekarang, bukan cuma test, karena memang belum ada provider asli sama sekali (bukan lupa, keputusan scope dari awal Phase 9).
2. **`POST /payments/{payment_id}/webhook` PUBLIK, tanpa `getAuthContext` sama sekali** — keputusan desain eksplisit: provider asli akan otentikasi callback-nya lewat signature khusus provider (bukan bearer token user), yang belum ada buat diverifikasi karena belum ada provider asli. Bukan celah keamanan yang kelewat — di produksi nanti, provider asli WAJIB dapat verifikasi signature sebelum endpoint ini bisa dipakai sungguhan (dicatat sebagai syarat, bukan detail teknis kecil).
3. **Webhook idempotent by design** — `order.status !== 'pending'` → no-op, balikin state saat ini. Ini BUKAN cuma soal kebersihan data: webhook di dunia nyata bisa terkirim >1 kali (delivery "at-least-once" itu standar), dan tanpa guard ini tutor bisa dibayar 2x untuk 1 booking — bug finansial nyata, bukan sekadar redundant write kayak pola "assign berulang" di ticket lain.
4. **`cohorts.startsAt` kosong (belum dijadwalkan) → default 100% refund**, bukan 0% — tidak ada dasar buat platform menahan uang siswa kalau tutor sendiri belum commit ke jadwal pasti. Didokumentasikan sebagai default eksplisit, bisa direvisit.
5. **`orders.status` cuma jadi `refunded` kalau ADA uang yang benar-benar dikembalikan** — tingkat kebijakan <6 jam (0% refund) TIDAK mengubah status order (tetap `paid`), karena "refunded" berarti "uang sudah dikembalikan", bukan "sudah dicoba dibatalkan".
6. Split 30/70: bagian tutor di-`Math.round()`, fee platform = sisa hasil bagi (`amount_idr - tutorShare`) — bukan dibulatkan independen — jadi keduanya selalu pas totalnya `amount_idr` tanpa sisa rupiah yang hilang/nambah.
7. Auth checkout/cancel: "milik sendiri" murni (`ctx.userId === enrollment.studentId`), BUKAN lewat `permissions.ts` matrix — pola sama `enrollSelf` (P9-004).
8. `wallet_repository.ts` dan komentar di titik insert payout (`order_service.handleWebhook`) SAMA-SAMA mendokumentasikan ambiguitas unit `transactions.amount` yang sudah dicatat di "Keputusan scope" — Rupiah mentah buat `payout_earned`/`payout_withdrawn`, credit buat `earn`/`spend`/`purchase`/`refund` lain, TIDAK ada konversi antar keduanya.
