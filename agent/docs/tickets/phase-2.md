...
**Endpoint:** `POST /ai/generate-lesson`, `POST /ai/generate-questions` — internal/admin (curriculum_developer+, same gate as manual authoring), lewat AI Gateway yang sama (P1-011)
...
**Acceptance Criteria:**
- [x] `LessonGeneration`: AI keluarkan ALM (bukan Block JSON — aturan 2.2), lewat parser P2-006, hasilnya masuk status `draft` (**tidak pernah** auto-publish — aturan keras 2.6/ADR-0004)
- [x] `QuestionGeneration`: AI **wajib** keluarkan Semantic JSON langsung sesuai schema P2-004 (bukan ALM) — 2 jalur output berbeda ini benar-benar terpisah (`lesson_generation_prompt` vs `question_generation_prompt`, prompt & parsing sendiri-sendiri, tidak ada template gabungan)
- [x] Blueprint input (topik, grammar target, vocab target, jumlah soal per skill) sebagai struct/schema eksplisit (`LessonGenerationBlueprint`/`QuestionGenerationBlueprint`), bukan free-text prompt tak terstruktur
- [x] `ai_tasks.status=failed` (gagal validasi schema/constitution P2-011) tidak charge credit ke platform account manapun tanpa audit trail — tetap tercatat di `ai_tasks`, cuma tidak lanjut ke publish (task ini 0-credit either way, jadi "tidak charge" trivially benar untuk kedua status — yang penting baris `ai_tasks` selalu tercatat)
**DoD:** 10 integration test di `tests/ai_content_test.rs` (mock `FakeAIProvider`): generate lesson (valid → draft tersimpan lewat P2-006 parser + linked concepts), generate question (valid → tervalidasi P2-004), 4 skenario gagal (ALM malformed, grammar constitution kurang section, provider error, question count mismatch/schema invalid) tiap-tiap membuktikan **tidak ada** `lessons`/`questions` row yang setengah jadi + `ai_tasks.status=failed`, 2 test permission-check-sebelum-panggil-provider. **Smoke test manual ke DeepSeek/OpenRouter asli** untuk kedua endpoint: lesson non-grammar (lolos, blocks realistis termasuk `indonesian_learner_alert`/`think_in_english`/`flashcard`), lesson grammar (lolos 11-section Constitution di percobaan pertama), 3 soal mcq (semua valid, jawaban benar).
**Catatan implementasi:**
- **Percobaan pertama smoke test 2x gagal** (model coba pakai `:::audio:::`/`:::indonesian_learner_alert:::` tanpa syntax `key: value` yang benar) — bukan bug kode, `ai_tasks.status=failed` tercatat benar dan **tidak ada lesson row setengah jadi** di kedua kasus (dibuktikan langsung query DB). Diperbaiki dengan memperkuat system prompt: kasih contoh syntax persis tiap directive text-only yang boleh dipakai, dan eksplisit larang directive `audio`/`video`/`image`/`question_embed` (model tidak punya asset/question id asli untuk direferensikan). Setelah diperkuat, generate lesson non-grammar DAN grammar sukses di percobaan berikutnya. Ini bukti nyata kenapa validate-before-write (bukan cuma "assume AI ngikutin instruksi") itu wajib, bukan defensive coding berlebihan.
- Validasi AI-generated lesson terjadi **sebelum** ada row `lessons` sama sekali: `alm_parser::parse` (pure fn) → `content_block_service::validate_blocks` (baru, diekstrak dari `validate_and_replace_blocks` — P2-003's write path sekarang `validate_blocks` lalu `replace_content_blocks`, dipisah supaya bisa dipanggil tanpa `lesson_id`) → (kalau grammar) `curriculum_constitution::validate_grammar_lesson_typed` (baru, varian `validate_grammar_lesson` yang terima `(type, data)` pairs, bukan `ContentBlock` asli — dipakai karena belum ada row lesson buat bikin `ContentBlock` sungguhan). Baru kalau semua lolos, `content_service::create_lesson` dipanggil (re-parse deterministic, harusnya tidak pernah gagal beda dari pre-check di atas).
- Validasi AI-generated questions: parse JSON array → cek `items.len() == blueprint.count` → `question_schema::validate` tiap item **sebelum** insert manapun — 1 item gagal = seluruh batch gagal, tidak ada partial write (dibuktikan test `generate_questions_schema_failure_creates_no_rows`).
- `strip_code_fence` dipindah dari `ai_gateway_service.rs` ke `ai_provider.rs` (jadi `pub fn`) supaya dipakai bersama oleh `ai_content_service.rs` — bukan duplikasi fungsi 15 baris di 2 tempat.
- `Config` dapat 2 field baru: `ai_lesson_generation_model`, `ai_question_generation_model` (default `deepseek/deepseek-chat`, sama seperti `ai_grammar_evaluation_model`).
...
**Acceptance Criteria:**
- [x] Berjalan otomatis saat `submit-review` dipanggil (P2-005) — bukan proses terpisah yang harus dipicu manual (`content_service::submit_lesson_for_review`/`question_service::submit_for_review`)
- [x] Hasil QA (pass/fail + daftar issue) tersimpan/terlampir ke item yang direview (`qa_report JSONB` di `lessons`/`questions`, migration 0015) — dikembalikan langsung di response submit-review, dan di `GET /lessons/{id}`/`GET /question-banks/{id}/questions`, supaya human reviewer lihat langsung apa yang sudah dicek
- [x] QA fail **tidak** memblokir masuk `in_review` — dibuktikan 2 test (`lesson_submit_review_flags_cefr_mismatch_without_blocking`, `question_submit_review_flags_invalid_schema_without_blocking`) yang eksplisit assert `response.status() == 200` DAN `qa_report.passed == false` sekaligus
**DoD:** test tiap kategori pengecekan menghasilkan flag yang benar (`grammar_constitution` — lewat pure fn karena P2-011 sudah hard-block kasus ini di endpoint asli; `cefr_mismatch` dan `question_schema` — lewat endpoint submit-review sungguhan), + 3 test QA-pass tidak menghasilkan false-positive (non-grammar lesson, cefr tag cocok, cefr tag kosong/untagged) — 7 integration test di `tests/content_qa_test.rs` + 2 unit test di `content_qa_service.rs`.
**Catatan implementasi:**
- **3 dari 4 kategori QA di 2.14 diimplementasi** (`grammar_constitution`, `question_schema`, `cefr_mismatch`) — persis 3 kategori yang disebut eksplisit di DoD ticket ini. Kategori ke-4 ("konsistensi terminologi grammar") **sengaja tidak dibangun**: tidak ada vocabulary/spec konkret di dokumen sumber manapun untuk dicek terhadapnya (beda dari 3 lainnya yang masing-masing punya validator siap pakai) — bikin heuristik palsu cuma buat centang kotak lebih buruk daripada jujur bilang belum ada spesifikasinya. Di-flag eksplisit di kode (`content_qa_service.rs` module doc) dan di sini, bukan didiamkan.
- **`grammar_constitution` check pada dasarnya redundan-tapi-sengaja** dengan hard gate P2-011: lesson grammar yang gagal Constitution sudah ditolak `submit-review` (422) sebelum QA Agent sempat jalan, jadi lewat endpoint asli kategori ini akan selalu "pass" untuk lesson yang berhasil sampai QA. Tetap diimplementasi & dites (lewat `content_qa_service::run_lesson_qa` langsung, bukan endpoint) supaya laporan QA tetap lengkap sesuai yang diminta ticket, bukan diam-diam skip kategori yang "sudah dicek di tempat lain".
- `cefr_mismatch` butuh kolom baru `questions.cefr_tag` (nullable, `a1`-`c2`, migration 0015 — belum ada endpoint authoring buat isi field ini, diset langsung untuk sekarang) — dibandingkan terhadap `levels.code` lesson yang meng-`question_embed` question itu (via `unit_id → level_id`). Tag kosong tidak pernah jadi false-positive.
